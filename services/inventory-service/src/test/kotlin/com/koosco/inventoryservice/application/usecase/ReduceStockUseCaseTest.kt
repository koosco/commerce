...
@DisplayName("ReduceStockUseCase")
class ReduceStockUseCaseTest {
    ...
    @InjectMocks
    lateinit var useCase: ReduceStockUseCase

    private fun command(skuId: String = "SKU-001", quantity: Int = 10, idempotencyKey: String? = null) =
        BulkReduceStockCommand(
            items = listOf(BulkReduceStockCommand.ReducingStockInfo(skuId, quantity)),
    ...
    inner class WithoutIdempotencyKey {
        ...
        fun `재고 감소 성공`() {
            ...
            verify(inventoryStockStore).decrease(any())
    ...
    inner class WithIdempotencyKey {
        ...
        fun `이미 처리된 키이면 아무 것도 하지 않는다`() {
            ...
            verify(inventoryStockStore, never()).decrease(any())
