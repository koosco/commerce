package com.koosco.catalogservice.application.usecase.snap
    ...
    fun execute(command: CreateSnapCommand, idempotencyKey: String? = null): SnapResult {
        if (idempotencyKey != null) {
            ...
            if (existing != null) {
                ...
                return SnapResult.Companion.from(snap)
            }
        }

        val snap = Snap.Companion.create(
        ...
        if (idempotencyKey != null) {
            catalogIdempotencyRepository.save(
                CatalogIdempotency.Companion.create(idempotencyKey, "SNAP", saved.id!!),
            )
        }

        return SnapResult.Companion.from(saved)
    }
}
