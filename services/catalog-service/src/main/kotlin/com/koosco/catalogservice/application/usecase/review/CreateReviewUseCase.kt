package com.koosco.catalogservice.application.usecase.review
    ...
    fun execute(command: CreateReviewCommand, idempotencyKey: String? = null): ReviewResult {
        if (idempotencyKey != null) {
            ...
            if (existing != null) {
                ...
                return ReviewResult.Companion.from(review)
            }
        }

        val review = Review.Companion.create(
        ...
        if (idempotencyKey != null) {
            catalogIdempotencyRepository.save(
                CatalogIdempotency.Companion.create(idempotencyKey, "REVIEW", saved.id!!),
            )
        }

        updateProductReviewStatistics(command.productId)

        return ReviewResult.Companion.from(saved)
    }
