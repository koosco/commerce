package com.koosco.catalogservice.application.usecase.discount
...
class CreateDiscountPolicyUseCase(
    ...
    fun execute(command: CreateDiscountPolicyCommand): DiscountPolicyResult {
        ...
        val discountPolicy = DiscountPolicy.Companion.create(
            product = product,
            name = command.name,
        ...
        return DiscountPolicyResult.Companion.from(saved, now)
    }
        ...
        product.skus.filter { it.isActive() }.forEach { sku ->
            integrationEventProducer.publish(
                PriceChangedEvent(
                    ...
                    reason = PriceChangedEvent.Companion.REASON_DISCOUNT_APPLIED,
                    changedAt = now,
                ),
            )
        }
