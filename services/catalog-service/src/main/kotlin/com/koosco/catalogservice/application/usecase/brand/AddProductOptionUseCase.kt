package com.koosco.catalogservice.application.usecase.brand
...
class AddProductOptionUseCase(
    ...
    fun execute(command: AddProductOptionCommand): ProductInfo {
        ...
        val existingCombinations = product.skus
            .filter { it.isActive() }
            .map { ProductOptions.Companion.fromJson(it.optionValues) }
            .toSet()
        ...
        return ProductInfo.Companion.from(savedProduct)
    }
    ...
    private fun generateAllCombinations(product: Product): List<Pair<ProductOptions, Long>> {
        ...
        return cartesianProduct(groups).map { combination ->
            ...
            ProductOptions.Companion.from(optionsMap) to additionalPrice
        }
    }
