...
/**
 * fileName       : TestInventoryConsumer
 * author         : koo
 * date           : 2025. 12. 26. 오전 7:40
 * description    : 재고 이벤트 발행 테스트를 위한 Consumer, local profile only
 */
@Profile("local")
@Component
class TestInventoryConsumer(private val objectMapper: ObjectMapper) {
    ...
    @KafkaListener(
        ...
        groupId = "order-service-test-inventory",
        containerFactory = "kafkaListenerContainerFactory",
    )
    fun onOrderPlaced(event: CloudEvent<*>) {
        logger.info("✅ [TEST-INVENTORY] OrderPlaced received: eventId=${event.id}")
    ...
    fun onOrderCancelled(event: CloudEvent<*>) {
        logger.warn("❌ [TEST-INVENTORY] OrderCancelled received: eventId=${event.id}")
    ...
    fun onOrderConfirmed(event: CloudEvent<*>) {
        logger.info("✅ [TEST-INVENTORY] OrderConfirmed received: eventId=${event.id}")
    ...
    fun clearReceivedEvents() {
        ...
        logger.info("🧹 [TEST-INVENTORY] All received events cleared")
    }
}
