 ...
 *
 * 목적: 고부하 환경에서 상품 상세 조회 시스템 한계 및 안정성 검증
 * - 대량의 동시 조회 요청 처리 능력 테스트
 * - 시스템 Breaking Point 탐색
 */

// Breaking Point: > 800 VU (estimated, single product read)
const B = 800;
    ...
    check(res, {
      'success response valid': (r) => {
        try {
          ...
          return body.success === true && body.data && body.data.productId;
        } catch (e) {
...
export function handleSummary(data) {
  const html = generateHTMLReport(data, {
    title: 'Stress Test - Product Detail',
    theme: 'stress',
  });

  return {
    [resultPath('results/catalog/product-detail/stress.test.result.html')]: html,
