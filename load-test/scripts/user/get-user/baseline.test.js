...
/**
 * Baseline Test - Get User
 *
 * 목적: 정상 부하 환경에서 사용자 조회 성능 측정
 * - 일반적인 트래픽 패턴 시뮬레이션
 * - 성능 기준선(Baseline) 측정
 */
  ...
  if (res.status === 200) {
    ...
    check(res, {
      ...
      'has user data': (r) => {
        try {
          const body = JSON.parse(r.body);
          return body.data && body.data.id && body.data.email;
        } catch (e) {
          return false;
        }
      },
...
export function handleSummary(data) {
  const html = generateHTMLReport(data, {
    title: 'Baseline Test - Get User',
    theme: 'baseline',
  });

  return {
    'results/user/get-user/baseline.test.result.html': html,
