...
/**
 * Baseline Test - Get Order
 *
 * 목적: 정상 부하 환경에서 주문 상세 조회 성능 측정
 * - 일반적인 트래픽 패턴 시뮬레이션
 * - 성능 기준선(Baseline) 측정
 */
  ...
  if (res.status === 200) {
    ...
    check(res, {
      ...
      'has order data': (r) => {
        try {
          const body = JSON.parse(r.body);
          return body.data && body.data.orderId && body.data.status;
        } catch (e) {
          return false;
        }
      },
...
export function handleSummary(data) {
  const html = generateHTMLReport(data, {
    title: 'Baseline Test - Get Order',
    theme: 'baseline',
  });

  return {
    'results/order/get-order/baseline.test.result.html': html,
